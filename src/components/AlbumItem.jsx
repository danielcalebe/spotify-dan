...
import { PlayerContext } from '../context/PlayerContext';
  ...
  const AlbumItem = ({ image, name, desc, id }) => {
    ...
    const [isPlaying, setIsPlaying] = useState(false);
      const { playWithId, track, playStatus, play } = useContext(PlayerContext);
    ...
    return (
      ...
      {/* Imagem com o botão de play/pause */}
        <div className="relative">
          ...
          >
            <img
              src={track.playStatus ? assets.pause_black_icon : assets.play_black_icon}
              alt={track.playStatus ? "Pause" : "Play"}
              className="w-4"
            />
          </div>
        </div>
